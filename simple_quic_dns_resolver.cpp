...
int recv_stream_data(ngtcp2_conn *conn, uint32_t flags, int64_t stream_id,
  ...
  fprintf(stdout, "Receiving stream data on stream id: %ld of length %ld \n", stream_id, datalen);

  // int ret = SimpleDNS::ParseDnsResponsePacket(buf, datalen);
  // if (ret != 0) {
  //   fprintf(stdout, "DNS Response Parsing error! \n");
  //   // return -1;
  // }
  int ack_rv = ngtcp2_acked_stream_data_offset(c->conn, stream_id, offset, datalen, user_data, stream_user_data);
  if (ack_rv != 0) {
    fprintf(stdout, "Error while sending stream ACK! \n");
  }

  fprintf(stdout, "Closing stream %ld! \n", stream_id);
  int stream_close_rv = ngtcp2_stream_close(c->conn, NGTCP2_STREAM_CLOSE_FLAG_NONE, stream_id, NULL, user_data, stream_user_data);
...
static size_t client_get_message(struct client *c, int64_t *pstream_id,
  ...
  fprintf(stdout, "Client get message: stream-id %ld\n", c->stream.stream_id);
